...
int main(int argc, char **argv) {
    ...
    gspan.maxpat= maxpat;
    gspan.maxitr= maxitr;
    gspan.scaling = scale; // standardize?
    gspan.gamma = 0.0;
    gspan.sigma = 0.1;
    gspan.c     = 0.5;
    gspan.alpha = 1.0;
    ...
    outfile << gspan.param.theta[0] << std::endl;
    //for(int s=1; s <= (int) gspan.param.get_num_vars(); ++s){
    for(set<int>::iterator it = gspan.param.nonzero_idx.begin(); it != gspan.param.nonzero_idx.end(); ++it){
      outfile << gspan.param.theta[*it]*scale << " " << gspan.param.retrieve(*it) << std::endl;
    }
